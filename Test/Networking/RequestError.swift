...
public enum RequestError: Error, Equatable {
    ...
    public static func == (lhs: RequestError, rhs: RequestError) -> Bool {
        switch (lhs, rhs) {
        case (.customMessage(let a), .customMessage(let b)) where a == b: return true
        case (.statusCode(let a), .statusCode(let b)) where a == b: return true
        case (.customError, .customError): return true
        case (.unknown, .unknown): return true
        case (.noResponse, .noResponse): return true
        case (.invalidURL, .invalidURL): return true
        case (.loadDataLocally, .loadDataLocally): return true
    ...
    public var message: String {
        ...
        case .decode:
            return "Decode error"
        case .loadDataLocally:
            return "Load data locally error"
        case .customError(let error):
            return error.localizedDescription
        case .statusCode(let statusCode):
            return FXDStatusCode(code: statusCode).description
...
public extension HTTPURLResponse {
    var status: FXDStatusCode? {
        return FXDStatusCode(rawValue: statusCode)
    }
}

public enum FXDStatusCode: Int {

    public init(code: Int) {
        self = FXDStatusCode(rawValue: code) ?? FXDStatusCode.unknown
    }
    ...
    /// - earlyHints: The HTTP 103 Early Hints information response status code is primarily intended to be used with the Link header to allow the user agent to start preloading resources while the server is still preparing a response.
    case earlyHints = 103
    ...
    /// - tooEarly: The HyperText Transfer Protocol (HTTP) 425 Too Early response status code indicates that the server is unwilling to risk processing a request that might be replayed, which creates the potential for a replay attack.
    case tooEarly = 425
